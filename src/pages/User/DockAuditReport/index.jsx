...
import { DaDataApi, DaPartNumberApi } from "../../../redux/action/userAction";
import CommonTable from "../../../components/common/commonTable";
import { DockAuditReportFooter, DockAuditReportHeader, DockAuditReportHeader as initialHeaders } from "../../../utils/constants/tableData";
...
const DockAuditReport = () => {
  ...
  const { DaPartNumberDetail, DaDataDetail } = useSelector(userSelector);
  ...
  const partNumberOptions = (DaPartNumberDetail?.varieties || [])?.map((item) => ({
    value: item,
    label: item,
  }));
  ...
  useEffect(() => {
    if (selectedShift && selectedDate) {
      ...
      dispatch(DaPartNumberApi(payload));
    }
  ...
  const handleViewTable = () => {
    ...
    dispatch(DaDataApi(payload))
  };


  useEffect(() => {
    if (DaDataDetail && DaDataDetail.Data && DaDataDetail.Data.length > 0) {
      const response = DaDataDetail.Data;
      setApiResponse(response);

      // Extract first item and required details
      const name = DaDataDetail?.PRODUCT || " ";
      ...
      const formNumber = DaDataDetail.Form_No || " ";
      ...
      const updatedHeaders = DockAuditReportHeader.map((row, rowIndex) => {
